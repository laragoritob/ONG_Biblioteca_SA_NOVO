...
document.addEventListener('DOMContentLoaded', function () {
 ...
 // AUTOCOMPLETE CLIENTE / DOADOR (compatível com buscar_clientes.php que retorna cod_cliente + nome)
document.addEventListener('DOMContentLoaded', function() {
    const doadorInput = document.getElementById('nome'); // ajuste para 'nome_cliente' se preferir
    const doadorSuggestions = document.getElementById('cliente-suggestions');
    const codDoadorInput = document.getElementById('cod_cliente'); // hidden input necessário
    let selectedIndex = -1;
  
    if (!doadorInput || !doadorSuggestions || !codDoadorInput) return;
    ...
    doadorInput.addEventListener('input', function() {
      ...
      fetch(`buscar_clientes.php?termo=${encodeURIComponent(termo)}`)
        ...
        .then(data => {
          if (data.error) { console.error('Erro (backend):', data.error); render([]); return; }
          render(data);
        })
        .catch(err => {
          console.error('Erro na requisição buscar_clientes.php:', err);
          render([]); // limpa sugestões
        });
    });
    ...
    doadorInput.addEventListener('keydown', function(e) {
      ...
      if (e.key === 'ArrowDown') {
        ...
        e.preventDefault();
        if (selectedIndex >= 0 && sugestoes[selectedIndex]) select(sugestoes[selectedIndex]);
    ...
    document.addEventListener('click', function(e) {
      if (!doadorInput.contains(e.target) && !doadorSuggestions.contains(e.target)) hide();
    });
  
    function render(items) {
      doadorSuggestions.innerHTML = '';
      if (!items || items.length === 0) { hide(); return; }
      items.forEach(item => {
        const div = document.createElement('div');
        div.className = 'autocomplete-suggestion';
        div.textContent = item.nome;
        // usa exatamente o campo retornado pelo seu PHP: cod_cliente
        div.dataset.codigo = item.cod_cliente;
        div.dataset.nome = item.nome;
        div.addEventListener('click', () => select(div));
        doadorSuggestions.appendChild(div);
      });
      doadorSuggestions.style.display = 'block';
      selectedIndex = -1;
    }
  
    function select(el) {
      doadorInput.value = el.dataset.nome;
      codDoadorInput.value = el.dataset.codigo;
      hide();
    }
  
    function update() {
      const sugestoes = doadorSuggestions.querySelectorAll('.autocomplete-suggestion');
      sugestoes.forEach((s, i) => s.classList.toggle('highlighted', i === selectedIndex));
    }
