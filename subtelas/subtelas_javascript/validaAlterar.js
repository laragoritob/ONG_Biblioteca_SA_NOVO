// ANIMAÇÃO DO BOTÃO DE ALTERAR - Validação específica para funcionários
document.getElementById('btnAlterar').addEventListener('click', function (e) {
    // Verifica se é um formulário de funcionário (tem campo usuario)
    const form = document.querySelector('#form_pessoal');
    const usuarioInput = form.querySelector('input[name="usuario"]');
    
    if (usuarioInput) {
        // É formulário de funcionário - aplica validação específica
    ...
    // Se não é formulário de funcionário, não faz nada (deixa o validaFormulario() do script interno cuidar)
});
