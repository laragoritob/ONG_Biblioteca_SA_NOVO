...
if (isset($_GET['id'])) {
    ...
    if ($confirmado) {
        // Executar a exclusão
        try {
            // Primeiro verificar se a editora existe
            $sql_check = "SELECT Nome_Editora FROM editora WHERE Cod_Editora = :id";
            $stmt_check = $pdo->prepare($sql_check);
            $stmt_check->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt_check->execute();
            
            $editora = $stmt_check->fetch(PDO::FETCH_ASSOC);
    ...
    $erro = "ID da editora não informado!";
}
?>
...
<head>
    ...
    <title>ONG Biblioteca - Excluir Editora</title>
            ...
            <?php if (!isset($sucesso) && !isset($erro)): ?>
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        window.location.href = 'excluir_editora.php?id=<?= $id ?>&confirmado=true';
                    } else {
